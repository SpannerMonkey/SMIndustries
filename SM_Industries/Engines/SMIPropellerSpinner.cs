...
{
    public class SMIPropellerSpinner : PartModule
    {
        [KSPField]
        public string engineID;
		[KSPField]
        public string primaryEngineID; // multmode hook up ?
		[KSPField]
       public string secondaryEngineID;// multmode hook up ?
        [KSPField]
        ...
        private SMIndustries.engine.SMIengineWrapper engine;
        ...
        private SMIanimateGeneric deployAnimation = new SMIanimateGeneric();
        ...
        public override void OnStart(PartModule.StartState state)
        {
            ...
            if (engineID == String.Empty || engineID == "")
                engine = new SMIndustries.engine.SMIengineWrapper(part);
            else
                engine = new SMIndustries.engine.SMIengineWrapper(part, engineID);

            if (engine.type == SMIengineWrapper.EngineType.SMIengine)
            {
                rotationSpeed = engine.smiengine.maxRPM;
            }
                ...
                else
                {
                    ...
                    if (rotorDisc != null)
                    {
                        ...
                        for (int i = 0; i < bladeNames.Count; i++)
                        {
                            ...
                            {
                                Debug.Log("SMIPropellerSpinner: Unable to find blade called " + bladeNames[i] + ", disabling swap");
                                useRotorDiscSwap = 0;
                            }
                        }
                    }
                    ...
                    {
                        Debug.Log("SMIplanePropellerSpinner: Unable to find rotor disc " + rotorDiscName + ", disabling swap");
                        useRotorDiscSwap = 0;
                    } 
                }
            ...
            {
                deployAnimation = part.Modules.OfType<SMIanimateGeneric>().FirstOrDefault();
            }
        }
        ...
        public override void OnUpdate()
        {
            ...
            if (engine != null)
            {
                ...
                {
                    rotationSpeed = engine.smiengine.maxRPM;
                    currentRPM = engine.smiengine.RPM / engine.smiengine.maxRPM;
                ...
                if (currentRPM != 0f)
                {
                    ...
                    if (useRotorDiscSwap == 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                setBladeRendererState(false);
                                propeller.Rotate(Vector3.forward * ((rotorDiscSpeed * (1 + (5 * engine.smiengine.RPMnormalized))) * TimeWarp.deltaTime));
                            }
                            else
                            {
                                setBladeRendererState(true);
