...
{
    public class SMIengine : PartModule, IThrustProvider
    {
        /// <summary>
        ...
        public enum SMIengineType
        {
            normal,
            bladed
        }

        public SMIengineType type = SMIengineType.normal;
        ...
        protected List<SMIresource> resourceList = new List<SMIresource>();
        
        private SMIndustries.info.SMIdebugMessages debug;
        ...
        protected void fillResourceList(string resourceString)
        {
            ...
            for (int i = 0; i < keyString.Length; i++)
            {
                ...
                if (valueString.Length > 0)
                {
                    try
                    {
                        resourceList.Add(new SMIresource(valueString[0].Trim(' '), float.Parse(valueString[1])));
        ...
        public Vector3 GetVelocity(Rigidbody rigidbody, Vector3 refPoint) // from Ferram
        {
            ...
            {
                if (debugMode)
                    Debug.Log("SMIengineBladed GetVelocity Exception " + e.GetType().ToString());
            }
            return newVelocity;
        }
