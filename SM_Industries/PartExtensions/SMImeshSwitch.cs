...
{
    public class SMImeshSwitch : PartModule //, IPartCostModifier
    {
        [KSPField]
        ...
        private SMIfuelSwitch fuelSwitch;
        ...
        private void parseObjectNames()
        {
            ...
            if (objectBatchNames.Length < 1)
                debug.debugMessage("SMImeshSwitch: Found no object names in the object list");
            else
            {
                ...
                for (int batchCount = 0; batchCount < objectBatchNames.Length; batchCount++)
                {
                    ...
                    for (int objectCount = 0; objectCount < objectNames.Length; objectCount++)
                    {
                        ...
                        if (newTransform != null)
                        {
                            ...
                            debug.debugMessage("SMImeshSwitch: added object to list: " + objectNames[objectCount]);
                        }
                        else
                        {
                            debug.debugMessage("SMImeshSwitch: could not find object " + objectNames[objectCount]);
                        }
                    }
        ...
        private void switchToObject(int objectNumber, bool calledByPlayer)
        {
            ...
            if (updateSymmetry)
            {
                ...
                {
                    SMImeshSwitch[] symSwitch = part.symmetryCounterparts[i].GetComponents<SMImeshSwitch>();
        ...
        private void setObject(int objectNumber, bool calledByPlayer)
        {
            ...
            {
                debug.debugMessage("calling on SMIfuelSwitch tank setup " + objectNumber);
        ...
        public void initializeData()
        {
            if (!initialized)
            {
                debug = new info.FSdebugMessages(debugMode, "SMImeshSwitch");
                ...
                if (useFuelSwitchModule)
                {
                    fuelSwitch = part.GetComponent<SMIfuelSwitch>();
                    if (fuelSwitch == null)
                    {
                        useFuelSwitchModule = false;
                        debug.debugMessage("no SMIfuelSwitch module found, despite useFuelSwitchModule being true");
                    }
                }
                initialized = true;
            }
