...
{
    public class SMIfuelSwitch : PartModule, IPartCostModifier
    {
        [KSPField]
        ...
        {            
            ...
            if (HighLogic.LoadedSceneIsEditor)
            {
                for (int s = 0; s < part.symmetryCounterparts.Count; s++)
                {
                    ...
                    SMIfuelSwitch symSwitch = part.symmetryCounterparts[s].GetComponent<SMIfuelSwitch>();
                    if (symSwitch != null)
                    {
        ...
        private void setupTankList(bool calledByPlayer)
        {
            ...
            for (int tankCount = 0; tankCount < resourceTankArray.Length; tankCount++)
            {
                ...
                for (int amountCount = 0; amountCount < resourceAmountArray.Length; amountCount++)
                {
                    ...
                    {
                        Debug.Log("SMIfuelSwitch: error parsing resource amount " + tankCount + "/" + amountCount + ": '" + resourceTankArray[amountCount] + "': '" + resourceAmountArray[amountCount].Trim()+"'");
                    }
                }
            }
